...
```{r}
# ---- Plot 3: Evolution of Average Household Size (storm-style) ----
...
```{r}
# ---- Combine INCOME + RENT (and population if desired) ----
rent_income <- INCOME |>
  dplyr::select(GEOID, NAME, year, household_income) |>
  dplyr::left_join(
    RENT |> dplyr::select(GEOID, year, monthly_rent),
...
```{r}
library(DT)
# --- Prep for Step 6: build rent_income + metrics safely (run BEFORE your DT tables) ---
...
```{r}
# --- Join POPULATION and PERMITS by CBSA + year ---
...
# ---- Repair/ensure Task 5 metrics exist BEFORE Step 6 ----
library(dplyr); library(stringr)
...
# --- SAFETY: ensure rent/housing metrics exist and merge them ---
...
```
```{r}
# --- Build early/late metrics per CBSA with flexible windows ---
yimby_summary <- yimby_df %>%
  group_by(CBSA, NAME) %>%
  arrange(year, .by_group = TRUE) %>%
...
### Plot 1:
```{r}
# thresholds (use means; fall back to medians if needed)
...
```{r}
# ---- Millennial Attraction Index (MAI): age 25–34 share relative to national avg ----
...
```
```{r}
# ---- Ensure rent burden + housing growth metrics exist (repair-safe) ----
library(scales)
...
# Raw + standardized rent burden (50 = national avg in 2009)
if (!("rent_burden_raw" %in% names(rent_income))) {
  rent_income <- rent_income %>%
    mutate(rent_burden_raw = if_else(
      is.finite(household_income) & household_income > 0 & is.finite(monthly_rent),
...
if (!all(c("instantaneous_growth","rate_based_growth") %in% names(housing_growth))) {
  housing_growth <- housing_growth %>%
    mutate(
      instantaneous_growth = (new_housing_units_permitted / population) * 1000,        # per 1,000 residents
      rate_based_growth    = if_else(pop_growth_5yr > 0,
                                     new_housing_units_permitted / (pop_growth_5yr*100), # per 1% pop growth
                                     NA_real_)
    )
}
safe_scale01 <- function(x){
...
# ---- Plot 1: YIMBY quadrant with Millennial Appeal (color = avg_MAI) ----
ggplot2::ggplot(
  yimby_summary,
